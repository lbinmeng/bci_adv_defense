...
from utils.autoattack.other_utils import L0_norm, L1_norm, L2_norm
from utils.autoattack.checks import check_zero_gradients
...
class APGDAttack():
    ...
    def dlr_loss(self, x, y):
        ...
        if x.shape[1] < 3:
            return F.multi_margin_loss(x, y, reduce=False)
        else:
...
class APGDAttack_targeted(APGDAttack):
    ...
    def dlr_loss_targeted(self, x, y):
        ...
        if x.shape[1] < 4:
            return -1. * F.multi_margin_loss(x, y, reduce=False)
        else:
