...
def train(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            ema_train_loss, ema_train_acc, _ = eval(teacher_model.model,
                                                    criterion_cal,
                                                    train_loader)
            ema_test_loss, ema_test_acc, _ = eval(teacher_model.model,
                                                  criterion_cal, test_loader)

            logging.info(
                'Epoch {}/{}: train loss: {:.4f} train acc: {:.2f} | test loss: {:.4f} test acc: {:.2f}'
                '  ||  EMA : train loss: {:.4f} train acc: {:.2f} | test loss: {:.4f} test acc: {:.2f}'
                .format(epoch + 1, args.epochs, train_loss, train_acc,
                        test_loss, test_acc, ema_train_loss, ema_train_acc,
                        ema_test_loss, ema_test_acc))
    ...
    for eps in [0.01, 0.05, 0.1]:
        adv_x = attack_lib.PGD(teacher_model.model,
                               x_test,
                               y_test,
                               eps=eps,
                               alpha=eps / 10,
        ...
        adv_loader = DataLoader(dataset=TensorDataset(adv_x.cpu(), y_test),
                                ...
                                num_workers=1,
                                drop_last=False)
        _, adv_acc, _ = eval(teacher_model.model, criterion_cal, adv_loader)
        adv_accs.append(adv_acc)
        logging.info(f'test acc: {test_acc}, PGD {eps} adv acc: {adv_acc}')
    for eps in [0.01, 0.05, 0.1]:
        adv_x = attack_lib.FGSM(teacher_model.model, x_test, y_test, eps=eps)
        adv_loader = DataLoader(dataset=TensorDataset(adv_x.cpu(), y_test),
                                batch_size=args.batch_size,
                                shuffle=False,
                                num_workers=1,
...
if __name__ == '__main__':
    ...
    model_path = f'model/self_ensembel_AT/target/{args.dataset}/{args.model}/{args.setup}'

    npz_path = f'result/npz/self_ensembel_AT/{args.setup}_{args.dataset}_{args.model}'
    if not os.path.exists(npz_path):
        os.makedirs(npz_path)
    npz_name = npz_path + '.npz'

    log_path = 'result/log/self_ensembel_AT'
