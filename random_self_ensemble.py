...
def train(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    for eps in [0.01, 0.05, 0.1]:
        ...
        adv_loader = DataLoader(dataset=TensorDataset(adv_x.cpu(), y_test),
                                ...
                                num_workers=1,
                                drop_last=False)
        _, adv_acc, _ = ensemble_eval(model, criterion, adv_loader)
...
if __name__ == '__main__':
    ...
    model_path = f'model/random_self_ensemble/target/{args.dataset}/{args.model}/{args.setup}'

    npz_path = f'result/npz/random_self_ensemble'
    if not os.path.exists(npz_path):
        os.makedirs(npz_path)
    npz_name = os.path.join(npz_path,
                            f'{args.setup}_{args.dataset}_{args.model}.npz')

    log_path = f'result/log/random_self_ensemble'
