...
def train(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    for epoch in range(args.epochs):
        ...
        for step, (batch_x, batch_y) in enumerate(train_loader):
            ...
            # print(f'loss_cal: {loss_cal.item()}, loss_scr: {loss_scr.item()}')
            if epoch <= 20: loss = loss_cal
            else: 
...
if __name__ == '__main__':
    ...
    model_path = f'model/SCR/target/{args.dataset}/{args.model}/{args.setup}'

    npz_path = f'result/npz/SCR/{args.setup}_{args.dataset}_{args.model}'
    if not os.path.exists(npz_path):
        os.makedirs(npz_path)
    npz_name = npz_path + '.npz'

    log_path = 'result/log/SCR'
